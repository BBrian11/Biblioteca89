...
export function useReservedLibros() {
    ...
    const [dateReserv, setdateReserv] = useState(localStorage.getItem("fechaReserva"));
    const [dateExpira, setdateExpira] = useState(localStorage.getItem("fechaExpira"));
    ...
    function datosLector() {
        myFirestore.collection('users').doc(`${codLector}`)
            .onSnapshot(snap => {
                setLector(snap.data().NombreApellido)
                setCi(snap.data().ci)
            })
    }
    ...
    useEffect(() => {
        ...
        let contraseña = "";
        for (let i = 0; i < 3; i++)contraseña += caracteres.charAt(Math.floor(Math.random() * caracteres.length));
    ...
    function registrarReserva() {
        ...
        .set({
            ...
            usuario: codLector,
            idBook: id,
            fechaReserva: dateReserv,
            horaReserva: hour,
            fechaExpira: dateExpira,
            entregado: false,
            expirado: false
        })
        ...
        return (
            <div className="main">
                ...
                <div className="subMain">
                    ...
                    <h4 style={{ textAlign: "center" }}>{hour}{horario} {dateExpira}</h4>
                    <h3>Usuario: </h3>
    ...
    return (
        <div className="row">
            ...
            <div className="col-lg-8">
                {localStorage.getItem("User") ? libLibros() : noExists()}
            </div>
            <div className="col-lg-2"></div>
        </div>
    )
